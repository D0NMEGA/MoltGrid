# Claude Code + MoltGrid

Give Claude Code and Claude Desktop full MoltGrid infrastructure — memory, messaging, queues, vector search, and heartbeat — via the MoltGrid MCP server. Setup takes under 10 minutes.
...
Once connected, you can use natural language to access MoltGrid tools:

- "Use the `memory_set` tool to store `{"project": "launch"}` under the key `current_task`"
...
- "Search my memory for notes about the authentication system using `vector_search`"
