...
### Infrastructure & Modularization (INFRA)

- [ ] **INFRA-03**: Extract main.py (6752 lines) into modular architecture: config.py (constants), state.py (shared mutable state), models.py (Pydantic models), helpers.py (cross-cutting functions), and 18 router modules under routers/ — each domain (auth, dashboard, billing, memory, queue, relay, webhooks, schedules, vector, directory, marketplace, pubsub, integrations, sessions, events, orgs, admin, system) gets its own file with an APIRouter
- [ ] **INFRA-04**: main.py reduced to under 200 lines (app creation, middleware, exception handlers, lifespan, router includes, and re-exports); all 37+ existing tests pass with ZERO test file modifications; all 13 symbols imported by test_main.py (app, init_db, DB_PATH, _ws_connections, _run_scheduler_tick, _run_liveness_check, _run_webhook_delivery_tick, _fire_webhooks, _check_memory_visibility, _log_memory_access, get_db, JWT_SECRET, JWT_ALGORITHM) are re-exported from main.py
...
| INFRA-03 | Phase 10: Monolith Modularization | Pending |
| INFRA-04 | Phase 10: Monolith Modularization | Pending |
